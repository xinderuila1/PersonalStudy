...
jira = JIRA(server='http://pm.glodon.com/newjira/', basic_auth=('sunjj', '52zhaodan!'))
...
g_CrashListAddress = 'http://crash.glodon.com/api/signature/%s/%s/crashes?from=all&to=all&ver_id=%s&m_ver=%s'
...
def crashInfoJson(sCrashListAddr):
        ...
        searchResult = re.findall(r"(\d+)", sCrashListAddr)
        if searchResult:
                crashList = g_CrashListAddress % (searchResult[0], g_ProductCode, searchResult[2], searchResult[2])
                result = urllib2.urlopen(g_CrashListAddress % (searchResult[0], g_ProductCode, searchResult[2], searchResult[2]))
...
def updateIssueInfo(sInfoDict):
        ...
        if sInfoDict:
                if sInfoDict['customfield_10801']:
                        #归属团队
                        g_Issue.update({'customfield_10801': {'id': sInfoDict['customfield_10801']}})
                ...
                if sInfoDict['autoTest']:
                        #是自动化
                        g_Issue.fields.labels.append(u'自动化')
...
def analysisIssue():
        global g_Issue
        crashListAddress = g_Issue.fields.customfield_10901
...
def beforeAnalysis(sExePath):
        ...
        global g_ProductVersion
        g_ProductVersion = g_Issue.fields.customfield_10212
#//////////////////////////////////////////
...
#for test
#searchIssue("GTJTJ-1875")
#beforeAnalysis(curPythonFilePath())
#analysisIssue()
#sMap ={'customfield_10801':'11117', 'assignee':'zhangyc-c', 'autoTest':'', 'description':'', 'remark':'', 'dumpUrl':'', 'highFrequency':''}
#updateIssue(sMap)
#afterAnalysis()
#searchCrashInfo("project in (GTJTJ, GTJJM, GYJL, JIRA) AND issuetype = Crash AND 测试版本号 ~ 1.0.9.0 AND assignee in (liuxs-a) ORDER BY priority DESC, updated DESC")
#print "hello"